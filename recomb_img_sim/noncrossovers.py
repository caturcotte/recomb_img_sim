...
def mask_nco_site(
    ...
    Args:
        ...
        nco_read_mask (np.ndarray, dtype=bool): 3D array where reads
            containing NCOs are True (arr[img, :, read] = True).
        nco_locations (np.ndarray, dtype=bool): 3D array with the NCO
            location for each image (arr[img, co_location, :] = True).

    Returns:
        nco_mask (np.ndarray, dtype=bool): 3D array where sites of NCOs on
            reads with NCOs are True (arr[img, co_location, read] = True).
    """
...
def put_nco_colors_in_array(
    ...
    Args:
        imgs (np.ndarray, dtype=int): The image array.
        nco_mask (np.ndarray, dtype=bool): 3D array with sites of NCOs on
            NCO reads = True (arr[img, nco_site, nco_read] = True).
        nco_read_mask (np.ndarray): 3D array of the same shape as imgs with
    ...
    Returns:
        imgs (np.ndarray, dtype=int): The image array, now with noncrossovers
            added.
    """
    match background:
        case "homozygous":
